...
//||                                             ||
//||=============================================||
// 预积分指的是对两个Lidar关键帧之间的imu数据进行预积分，用于估计bias
...
public:
    ...
    Preintegration(const Vector3d &acc0, const Vector3d &gyr0,
        ...
        // 重力向量
        g_vec_ = -Eigen::Vector3d(0, 0, 9.805); // ENU型
        // 观测噪声矩阵
    ...
    void MidPointIntegration(double dt,
        ...
        // 利用姿态更新前的q和acc,得到上一帧imu数据时刻对应的acc_n
        ...
        // 利用姿态更新后的q，以及当前帧imu的acc ，得到当前帧imu数据的acc_n
    ...
    // 计算imu预积分测量残差 +　bias更新的残差(liom在滑窗内优化时，会对每相邻两个优化位姿之间计算imu预积分残差,见Estimator::SolveOptimization())
    Matrix<double, 15, 1> evaluate(const Vector3d &Pi,
        ...
        residuals.setZero();
        // 认为更新的时候，会调整ba和bg，因此预积分误差对ba和bg的雅克比也需要调整。另外，ba,bg的线性化点也调整
    ...
    double dt_;
    Vector3d acc0_, gyr0_;      // 存储积分起点
    Vector3d acc1_, gyr1_;      // 存储当前帧imu的acc、gyr

    const Vector3d linearized_acc_, linearized_gyr_;    // 初始化的acc和gyr
    // 零偏无法通过imu测量获得，需要通过Lidar帧估计得到
    Vector3d linearized_ba_, linearized_bg_;            // ba、bg分别是随时间缓慢变化的加速度计、陀螺仪bias
    ...
    std::vector<double> dt_buf_;    // 接受imu时间间隔数组
    std::vector<Vector3d> acc_buf_; // 接受imu线性加速度数组
    std::vector<Vector3d> gyr_buf_; // 接受imu角加速度数组
